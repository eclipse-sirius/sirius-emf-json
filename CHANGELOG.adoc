...
- Add an optional, reproducible SysML serialization benchmark with allocation measurements and isolated JFR recordings.
- Reduce JSON save allocations by buffering character encoding when no resource handler observes the output stream.
- Reduce JSON save time by avoiding per-write synchronization in the private character buffer.
- Reduce JSON save allocations by pre-sizing arrays from known reference-list sizes.
- Add isolated codec-phase diagnostics and campaign-aware CPU/allocation profile analysis.
- Add an EMF binary save diagnostic for comparing codec time, allocation and output size.
- Add a JSON streaming-save diagnostic with JSON-tree and model-equivalence checks.
- Reduce allocations when assigning an unchanged object ID while preserving ID manager callbacks.
- Reduce save overhead for excluded features of frozen EClasses while preserving dynamic options and custom metadata behavior.
- Reduce save dispatch overhead with session-scoped plans for frozen standard EClass features.
- Add opt-in streaming JSON saves to reduce allocations when object-tree callbacks are not needed.
- Avoid temporary JSON primitives for single-valued strings, booleans and numbers in streaming saves.
- Reuse parsed class-name components while loading repeated EObject types.
- Add `JsonResource.loadFromString` to avoid an intermediate UTF-8 byte array.

=== Architectural decision records

- link:doc/adrs/001-streaming-json-save.adoc[ADR-001] Explore optional streaming JSON saves with an explicit object-callback restriction.
...
- [releng] Switched to https://download.eclipse.org/modeling/emf/emf/builds/release/2.46.0/index.html[Eclipse EMF 2.46.0].
...
- [releng] Switched to https://github.com/google/gson/releases/tag/gson-parent-2.13.2[Google GSON 2.13.2].

=== Breaking changes

- https://github.com/eclipse-sirius/sirius-emf-json/issues/79[#79]  Sirius EMF JSON now requires Java 21 or later to build and to run.

=== New features and improvements

- https://github.com/eclipse-sirius/sirius-emf-json/issues/73[#73] Generate and publish SBOMs
