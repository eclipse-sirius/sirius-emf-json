...
/**
 * Checks reference classification and callbacks that move reference targets between resources.
 */
public class SerializeManyReferencesTests {
    ...
    @Test
    public void resourceFragmentCallbackCanChangeReferenceClassification() {
        var external = this.resource("external"); //$NON-NLS-1$
        var type = this.type();
        var first = this.object(type, external, "first"); //$NON-NLS-1$
        ...
        var resource = new JsonResourceImpl(URI.createURI("sirius:///local"), Map.of()) { //$NON-NLS-1$
            ...
            public String getURIFragment(EObject object) {
                String fragment = super.getURIFragment(object);
                if (object == first) {
                    external.getContents().add(second);
                }
                return fragment;
            }
        };
    ...
    @Test
    public void mixedReferencesReclassifyAfterUriCallback() {
        var resource = this.resource("local"); //$NON-NLS-1$
        var external = this.resource("external"); //$NON-NLS-1$
        var type = this.type();
        var source = this.object(type, resource, "source"); //$NON-NLS-1$
    ...
    @Test
    public void customHelperCanChangeReferenceClassification() {
        var resource = this.resource("local"); //$NON-NLS-1$
        var external = this.resource("external"); //$NON-NLS-1$
        var type = this.type();
        var source = this.object(type, resource, "source"); //$NON-NLS-1$
        ...
        var helper = new JsonHelper(resource) {
            ...
            public String getIDREF(EObject object) {
                String result = super.getIDREF(object);
                external.getContents().add(second);
                return result;
            }
        };
    ...
    @Test
    public void emptyReferencesDoNotTriggerClassification() {
        var resource = this.resource("local"); //$NON-NLS-1$
        var source = this.object(this.type(), resource, "source"); //$NON-NLS-1$
        var referenceReads = new AtomicInteger();
        var helper = new JsonHelper(resource) {
            @Override
            public Object getValue(EObject object, EStructuralFeature feature) {
                if (feature instanceof EReference) {
                    referenceReads.incrementAndGet();
                }
                return super.getValue(object, feature);
            }
        };
        var values = this.serialize(resource, source, Map.of(JsonResource.OPTION_CUSTOM_HELPER, helper, JsonResource.OPTION_SAVE_UNSETTED_FEATURES, true));
        Assert.assertEquals(0, values.size());
        Assert.assertEquals(1, referenceReads.get());
    }
